...
export async function GET(req: NextRequest) {
    ...
    const [paymentsReceived, paymentsSent] = await Promise.all([
        db.paymentLink.findMany({
            where: { recipientAddress: address, status: { in: ["PAID", "COMPLETED"] } },
            orderBy: { createdAt: "desc" },
        }),
        db.paymentLink.findMany({
            where: { paidBy: address, status: { in: ["PAID", "COMPLETED"] } },
            orderBy: { createdAt: "desc" },
        }),
    ]);
