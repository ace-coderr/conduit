...
export interface PaymentOptions {
    ...
    payTo?: string;
    /** Facilitator URL. Defaults to Conduit's facilitator */
    facilitatorUrl?: string;
    /** Network. Defaults to Arc Testnet */
    network?: string;
    /** USDC contract address. Defaults to Arc Testnet USDC */
    asset?: string;
    /** Description shown in 402 response */
    description?: string;
    /** Max timeout in seconds. Defaults to 300 */
...
// ─── Helper: convert USDC decimal to atomic units ─────────────────────────────
...
function build402(req: NextRequest, opts: Required<PaymentOptions>, isHuman: boolean): NextResponse {
    const atomicAmount = toAtomicUnits(opts.amount);
    ...
    const paymentDetails = {
        ...
        description: opts.description || `Pay ${opts.amount} USDC to access this resource`,
    ...
    if (isHuman) {
        // Human-readable pay page
        const html = buildPayPage(opts.amount, opts.description || "Access this resource", opts.facilitatorUrl);
...
// ─── Helper: verify tx-based payment (human direct transfer) ─────────────────
...
async function verifyTxPayment(
    ...
    try {
        // Dynamic import to avoid bundling viem in middleware unnecessarily
        ...
        if (amount < required) {
            return { valid: false, error: `Insufficient amount: ${amount} < ${required}` };
        }
...
/**
 * Wrap any Next.js route handler with x402 USDC payment gate.
 *
 * @example
 * // app/api/my-endpoint/route.ts
 * import { withPayment } from "@/lib/x402Middleware";
 *
...
export function withPayment(
    ...
    return async (req: NextRequest, context?: any): Promise<NextResponse> => {
        ...
        // tx-based payment (human direct transfer)
        if (paymentSignature.startsWith("tx:")) {
            const txHash = paymentSignature.slice(3);
            const { valid, error } = await verifyTxPayment(txHash, opts);
            if (!valid) {
                console.log(`[x402] TX verify failed: ${error}`);
                return build402(req, opts, isHuman);
            }
            verified = true;
        } else {
            // EIP-3009 agent payment
            const { valid } = await verifyPayment(paymentSignature, opts, resourceUrl);
            if (!valid) {
                return build402(req, opts, isHuman);
            }
            verified = true;
            // Settle async — don't block the response
            settlePayment(paymentSignature, opts, resourceUrl);
        }

        if (!verified) return build402(req, opts, isHuman);
...
function buildPayPage(amount: string, description: string, facilitatorUrl: string): string {
...
<body>
  <div class="card">
    ...
    <button class="btn" id="payBtn" onclick="connectAndPay()">Connect Wallet & Pay ${amount} USDC</button>
    <div class="status" id="status"></div>
  </div>
  <p class="powered">Powered by <a href="https://conduit-pay.vercel.app">Conduit</a> · Built on Arc Network</p>
  <script>
  ...
  async function connectAndPay() {
    ...
    if (!window.ethereum) { status.textContent = 'MetaMask not found.'; status.className = 'status error'; return; }
    try {
      btn.disabled = true;
      ...
      try {
        await window.ethereum.request({ method: 'wallet_switchEthereumChain', params: [{ chainId: '0x4CE692' }] });
      } catch(e) {
        if (e.code === 4902) {
          await window.ethereum.request({ method: 'wallet_addEthereumChain', params: [{ chainId: '0x4CE692', chainName: 'Arc Testnet', nativeCurrency: { name: 'USD Coin', symbol: 'USDC', decimals: 18 }, rpcUrls: ['https://rpc.testnet.arc.network'], blockExplorerUrls: ['https://testnet.arcscan.app'] }] });
        }
      }
      status.textContent = 'Confirm payment in MetaMask...';
      const atomicAmount = Math.round(parseFloat('${amount}') * 1000000).toString();
      const txHash = await window.ethereum.request({
        method: 'eth_sendTransaction',
        params: [{ from: account, to: '${USDC}', data: encodeTransfer('${payTo}', atomicAmount) }]
      });
      status.textContent = 'Waiting for confirmation...';
      await waitForReceipt(txHash);
      status.textContent = 'Verifying payment...';
      const res = await fetch(window.location.href, { headers: { 'PAYMENT-SIGNATURE': 'tx:' + txHash } });
      if (res.ok) {
